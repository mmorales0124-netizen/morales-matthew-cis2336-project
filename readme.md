...
**A gallery and community platform for artists working in the dark, built around pure creative expression — built for CIS 2336.**

🔗 **Live Site:** https://mmorales0124-netizen.github.io/morales-matthew-cis2336-project/
🔗 **Repository:** https://github.com/mmorales0124-netizen/morales-matthew-cis2336-project
...
│       └── references.html
│
├── index.html                # redirects GitHub Pages root to frontend/index.html
...
1. **Clone the repository**
   \`\`\`
   git clone https://github.com/mmorales0124-netizen/morales-matthew-cis2336-project.git
   cd morales-matthew-cis2336-project
   \`\`\`

2. **Open the project in VS Code** (or your editor of choice).

3. **Run it with a local server**, rather than double-clicking the HTML file directly. The embedded YouTube video on the Gallery page requires a real \`http://\` origin to work correctly — opening the file directly (\`file://\`) can throw a player error.
...
4. Navigate the site using the nav bar — Home, Gallery, Events, Submit Art, FAQ, References.
...
> **To see the frontend and backend working together:** run both at the same time — start the backend first (\`npm start\` inside \`backend/\`), then open the frontend with Live Server. Go to the Submit Art page and submit the form; you'll get a real confirmation message back from the backend instead of a simulated one. You can verify the submission was stored by visiting \`http://localhost:3000/api/artworks\` in your browser.
...
Claude (Anthropic) was used throughout development to help design the visual direction, write HTML/CSS/JS, and debug issues (such as a YouTube embed player error). A full, chronological log of the prompts used is documented on the [References page](frontend/pages/references.html) under **AI Prompts Used During Development**, per the assignment's academic honesty requirement.
...
Artwork, fonts, and media credits are listed in full on the [References page](frontend/pages/references.html).
